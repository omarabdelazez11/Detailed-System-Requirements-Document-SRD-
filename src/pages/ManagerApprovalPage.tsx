...
import { ShieldCheck, Check, X, Clock, MapPin, Calendar, Star, AlertCircle, User, History, Inbox, Archive, Search, Filter } from 'lucide-react';
...
const ManagerApprovalPage: React.FC = () => {
  ...
  const filteredList = approvals.filter(a => {
    ...
    const matchesSearch = a.purpose.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          a.requester.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesRoom = roomFilter === 'All Rooms' || a.room === roomFilter;
    ...
    <div className="card" style={{ padding: '2.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, color: '#64748b', fontSize: '0.9rem' }}>Filter and manage high-priority campus decisions.</p>
          </div>
        </div>
        ...
        <div style={tabContainer}>
           ...
           <button onClick={() => setActiveTab('Declined')} style={tabBtn(activeTab === 'Declined', '#ef4444')}><History size={14} /> History</button>
        </div>
      </div>
      ...
      <div style={filterBar}>
         ...
         <div style={roomGroup}>
            ...
            <select value={roomFilter} onChange={(e) => setRoomFilter(e.target.value)} style={selectInput}>
               <option value="All Rooms">All Multi-Purpose Rooms</option>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gap: '1.5rem' }}>
        ...
        {filteredList.length === 0 && (
          <div style={emptyState}>
            ...
            <p>No results match your current filters in the "{activeTab}" section.</p>
          </div>
        )}
      </div>
    </div>
...
const tabBtn = (active: boolean, color: string) => ({ background: active ? `${color}22` : 'transparent', color: active ? color : '#64748b', border: active ? `1px solid ${color}33` : 'none', padding: '8px 16px', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' });
const filterBar = { display: 'flex', gap: '1.5rem', marginBottom: '2.5rem', background: '#0f172a', padding: '1.25rem', borderRadius: '20px', border: '1px solid #1e293b' };
const searchGroup = { flex: 2, display: 'flex', alignItems: 'center', gap: '12px', background: '#1e293b', padding: '0.75rem 1.25rem', borderRadius: '12px', border: '1px solid #334155' };
const searchInput = { background: 'transparent', border: 'none', color: 'white', outline: 'none', width: '100%', fontSize: '0.9rem' };
const roomGroup = { flex: 1, display: 'flex', alignItems: 'center', gap: '12px', background: '#1e293b', padding: '0.75rem 1.25rem', borderRadius: '12px', border: '1px solid #334155' };
const selectInput = { background: 'transparent', border: 'none', color: 'white', outline: 'none', width: '100%', fontSize: '0.9rem', cursor: 'pointer' };
const approvalCard = { background: '#0f172a', padding: '1.5rem', borderRadius: '24px', border: '1px solid #1e293b' };
const idBadge = { background: '#1e293b', color: '#64748b', padding: '4px 10px', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 'bold' };
const statusBadge = (s: string) => ({ background: s === 'Approved' ? '#22c55e11' : s === 'Declined' ? '#ef444411' : '#f59e0b11', color: s === 'Approved' ? '#22c55e' : s === 'Declined' ? '#ef4444' : '#f59e0b', padding: '4px 10px', borderRadius: '8px', fontSize: '0.7rem', fontWeight: 'bold' });
